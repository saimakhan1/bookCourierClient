...
//   // Fetch book details
//   useEffect(() => {
//     fetch(`https://book-courier-server.vercel.app/books/${id}`)
//       .then((res) => res.json())
...
//   useEffect(() => {
//     fetch(`https://book-courier-server.vercel.app/reviews/${id}`)
//       .then((res) => res.json())
...
//     try {
//       const res = await fetch("https://book-courier-server.vercel.app/orders", {
//         method: "POST",
...
//         "https://book-courier-server.vercel.app/reviews",
//         {
//           method: "POST",
//           headers: {
...
const BookDetails = () => {
    ...
    fetch(`https://book-courier-server.vercel.app/orders?email=${user.email}`)
      ...
      .then((orders) => {
        const ordered = orders.some(
          (order) =>
            order.bookId === id ||
            order.bookId === (book?._id ? book._id : id)
        );
        setHasOrdered(ordered);
      })
      .catch((err) => console.log(err));
  ...
  // 🔐 Order button handler (ONLY MODIFIED PART)
  ...
  const handlePlaceOrder = async (e) => {
    ...
    try {
      const res = await fetch(
        "https://book-courier-server.vercel.app/orders",
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(orderData),
  ...
  return (
    <div className="max-w-4xl mx-auto p-5 dark:text-white">
      <h2 className="text-xl text-blue-800 dark:text-gray-200 font-bold mb-7">
        If you buy a book (with payment), you will be able to post a review.
      </h2>
